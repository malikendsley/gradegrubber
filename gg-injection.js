...
window.addEventListener("load", () => {
  ...
  chrome.storage.sync.get(null, function (settings) {
    ...
    if (!settings.EXTENSION_ENABLED) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (rowType == "assignment") {
          ...
          if (settings.SHADE_GRADES) {
            ...
            let relativePerformance = getRelativePerformance(
              grades[curAssignment]["grade"],
              grades[curAssignment]["maxGrade"],
              grades[curAssignment]["averageGrade"]
            );
            let color = getBlendedColor(
              relativePerformance,
              settings.BAD_COLOR,
              settings.GOOD_COLOR,
      ...
      let isWeighted = document
        .getElementById("assignments-not-weighted")
        .innerHTML.includes("Assignments are weighted by group:");
      ...
      if (isWeighted) {
        ...
        [studentAvg, courseAvg, perCategoryPerformance] = gradeWeights
          ? calculateTotalGradesWeighted(grades, gradeWeights)
          : calculateTotalGradesUnweighted(grades);
        ...
        if (perCategoryPerformance && settings.SHADE_CATEGORIES) {
          ...
          for (let categoryStatEntry in perCategoryPerformance) {
            let relativePerformance = getRelativePerformance(
              perCategoryPerformance[categoryStatEntry]["personal"],
              perCategoryPerformance[categoryStatEntry]["maxPersonal"],
              perCategoryPerformance[categoryStatEntry]["average"]
            );
            let color = getBlendedColor(
              relativePerformance,
              settings.BAD_COLOR,
      ...
      let totalcourseColor = getBlendedColor(
      ...
      studentAvg *= 100;
      courseAvg *= 100;
      let personalScoreBar = document.querySelector(
        "#gg-personalBar-progress-bar"
      );
      ...
      document.querySelector("#gg-readout").innerHTML =
        "<p>Your grade is <b>" +
        Math.abs(studentAvg - courseAvg).toFixed(2) +
        "% " +
        (studentAvg > courseAvg ? "higher" : "lower") +
        "</b> than the class average. " +
        getCallToAction(studentAvg, courseAvg) +
        "</p>";
...
function parseRow(row) {
  let rowType = "skip";
  if (
    row.classList.contains("student_assignment") &&
    row.classList.contains("assignment_graded")
  ) {
    rowType = "assignment";
    //console.log("assignment row detected");
  } else if (
    row.classList.contains("comments") &&
    row.classList.contains("grade_details") &&
    row.classList.contains("assignment_graded")
  ) {
...
function extractAssignmentName(row) {
  try {
    return row.getElementsByTagName("th")[0].getElementsByTagName("a")[0]
      .innerHTML;
  } catch (e) {
...
function extractAssignmentType(row) {
  try {
    return row.getElementsByTagName("th")[0].getElementsByTagName("div")[0]
      .innerHTML;
  } catch (e) {
  ...
  try {
    ...
    if (score.includes("%")) {
      ...
      let spans = Array.from(
        row.querySelector(".tooltip").getElementsByTagName("span")
      );
      //console.log("debug" + spans.innerHTML);
      maxScore = spans
        .filter((span) => span.classList.length === 0)[0]
...
function extractGradeWeights() {
  try {
    ...
    for (let i = 0; i < gradeWeightsList.children.length - 1; i++) {
      ...
      gradeWeights[gradeWeightsList.children[i].children[0].innerHTML] =
        fraction;
    }
    return gradeWeights;
  } catch (e) {
...
function calculateTotalGradesWeighted(grades, weights) {
  ...
  let personalScore = 0,
    averageScore = 0;
  for (let type in typeScores) {
    if (typeScores[type].maxPersonal > 0) {
      // Making sure not to divide by zero
      // Adding the weighted personal score to the total personal score
      personalScore +=
        (typeScores[type].personal / typeScores[type].maxPersonal) *
        usedWeights[type];
    }
    if (typeScores[type].maxAverage > 0) {
      // Making sure not to divide by zero
      // Adding the weighted average score to the total average score
      averageScore +=
        (typeScores[type].average / typeScores[type].maxAverage) *
        usedWeights[type];
    }
  }
...
function getBlendedColor(relPerf, badColor, goodColor, neutralColor, sens) {
    ...
    interpolatedComponents = {
      r: Math.round(
        Math.max(
          ...
          Math.min(
            255,
            (1 + relPerf * sens) * neutralColor.r + -relPerf * sens * badColor.r
          )
        )
      ),
      g: Math.round(
        Math.max(
          ...
          Math.min(
            255,
            (1 + relPerf * sens) * neutralColor.g + -relPerf * sens * badColor.g
          )
        )
      ),
      b: Math.round(
        Math.max(
          ...
          Math.min(
            255,
            (1 + relPerf * sens) * neutralColor.b + -relPerf * sens * badColor.b
          )
        )
      ),
    };
    ...
    interpolatedComponents = {
      r: Math.round(
        Math.max(
          ...
          Math.min(
            255,
            (1 - relPerf * sens) * neutralColor.r + relPerf * sens * goodColor.r
          )
        )
      ),
      g: Math.round(
        Math.max(
          ...
          Math.min(
            255,
            (1 - relPerf * sens) * neutralColor.g + relPerf * sens * goodColor.g
          )
        )
      ),
      b: Math.round(
        Math.max(
          ...
          Math.min(
            255,
            (1 - relPerf * sens) * neutralColor.b + relPerf * sens * goodColor.b
          )
        )
      ),
    };
  ...
  let color =
    "#" +
    RGBComponentToHex(interpolatedComponents.r) +
    RGBComponentToHex(interpolatedComponents.g) +
    RGBComponentToHex(interpolatedComponents.b);
  return color;
}
